...
def main():
    parser = argparse.ArgumentParser(
        prog="ming", description="乾坤镜 CLI v0.11.9m — OpenClaw 监控与诊断工具"
    )
    parser.add_argument("--version", action="version", version="ming 0.11.9m")
    sub = parser.add_subparsers(dest="command")

    p_skill = sub.add_parser("skill", help="管理技能插件")
    ...
    p_dx = sub.add_parser("dx", help="诊断管理（列表/详情/确认/导出）")
    p_dx.add_argument(
        "query_name",
        nargs="?",
        help="查询名称（recent_p0/recent_p1/event_stats/system_stats/unconfirmed）",
    )
    p_dx.add_argument("--limit", type=int, default=20, help="返回条数上限（默认 20）")
    p_dx.add_argument("--json", action="store_true", help="JSON 输出")

    p_health = sub.add_parser("health", help="查看系统健康状态")
    p_health.add_argument("--json", action="store_true", help="JSON 输出")

    p_query = sub.add_parser("query", help="运行预定义查询（v0.11.9m 扩展）")
    p_query.add_argument("query_name", help="查询名称（可用查询见 --help）")
    p_query.add_argument("--limit", type=int, default=20, help="返回条数上限")
    p_query.add_argument("--json", action="store_true", help="JSON 输出")
    p_query.add_argument("-s", "--system", help="按系统名称过滤")
    p_query.add_argument("--since", type=float, help="起始时间戳（Unix epoch）")
    p_query.add_argument(
        "--group-by",
        choices=["model", "hour", "agent"],
        default="model",
        help="分组维度（token-breakdown 用）",
    )
    p_query.add_argument("--session-id", help="Session ID（step-sequence 用）")
    p_query.add_argument(
        "--window",
        type=int,
        default=600,
        help="时间窗口秒数（step-loop / token-spike 用）",
    )
    p_query.add_argument(
        "--threshold",
        type=float,
        default=3.0,
        help="突增倍数阈值（token-spike 用，默认 3.0）",
    )

    p_status = sub.add_parser("status", help="查看归档器运行状态")
    p_status.add_argument("--json", action="store_true", help="JSON 输出")

    p_self_check = sub.add_parser(
        "self-check", help="运行系统自检（退出码: 0=正常 1=警告 2=严重）"
    )
    p_self_check.add_argument("--json", action="store_true", help="JSON 输出")

    p_config = sub.add_parser("config", help="查看/验证配置")
    ...
    p_admin = sub.add_parser(
        "admin", help="管理命令（删除 session / 压缩数据库 / 清理测试数据）"
    )
    p_admin.add_argument("action", choices=["forget", "vacuum", "cleanup"])
    p_admin.add_argument("--session-id", help="要删除的 Session ID（forget 操作必需）")
    p_admin.add_argument(
        "--confirm", action="store_true", help="确认删除（forget 操作必需）"
    )

    p_archive = sub.add_parser("archive", help="管理冷轨归档文件")
    ...
    p_web.add_argument("--token", type=str, default=None, help="API 认证令牌（可选）")
    p_web.add_argument("--no-browser", action="store_true")

    p_triage = sub.add_parser("triage", help="运行分诊诊断")
    p_triage.add_argument("action", choices=["run", "status", "report"])
    p_triage.add_argument("--no-diagnose", action="store_true", help="跳过自动诊断")
    p_triage.add_argument("--json", action="store_true", help="JSON 输出")
    ...
    p_replay.add_argument("--system", "-s", required=True, help="系统名称")
    p_replay.add_argument(
        "--since", default="1h", help="起始时间（30m/2h/1d 或 Unix 时间戳）"
    )
    p_replay.add_argument("--until", default=None, help="结束时间（同上，默认 now）")
    p_replay.add_argument(
        "--detail", choices=["normal", "full"], default="normal", help="详细程度"
    )
    p_replay.add_argument("--json", action="store_true", help="JSON 输出")

    p_ignore = sub.add_parser("ignore", help="忽略某疾病（疾病仍然记录，仅报告不展示）")
    p_ignore.add_argument("fault_id", help="疾病规则ID")
    p_ignore.add_argument("-s", "--system", required=True, help="系统名称")

    p_archive_disease = sub.add_parser(
        "archive-disease", help="归档某疾病（永久隐藏，不计入健康）"
    )
    p_archive_disease.add_argument("fault_id", help="疾病规则ID")
    p_archive_disease.add_argument("-s", "--system", required=True, help="系统名称")

    p_restore = sub.add_parser("restore", help="取消忽略/取消归档")
    p_restore.add_argument("fault_id", help="疾病规则ID")
    p_restore.add_argument("-s", "--system", required=True, help="系统名称")

    p_reset = sub.add_parser(
        "reset", help="健康复位某实例（强制标记健康，新疾病出现取消）"
    )
    p_reset.add_argument("system", help="系统名称")

    p_reset_status = sub.add_parser("reset-status", help="查看健康复位状态")
    p_reset_status.add_argument("system", nargs="?", help="系统名称（不指定则全部）")

    sub.add_parser("instance-list", help="紧凑列出所有实例及其健康状态")

    sub.add_parser("hermes-install", help="一键安装乾坤镜探针到 Hermes Agent")

    p_exclude = sub.add_parser("exclude", help="停止/恢复观察某个实例")
    ...
    p_probe.add_argument("--force", action="store_true", help="跳过确认")
    p_probe.add_argument("--keep-data", action="store_true", help="保留数据库记录")
